...
from support import my_utils, image_transforms
from support.my_utils import save_image

class Parameters(object):
    def __init__(self, base_args):
        self.output_root = '../results/fuzz/'

        self.model = base_args.model
        self.metric = 'NCS'
        self.blind = base_args.blind

        self.batch_size = 1
        self.alpha = 0.02 # default 0.02
        self.beta = 0.2 # default 0.2
        self.TRY_NUM = 50
        self.save_every = 100
        ...
        self.num_perclass = 1000 if self.dataset == 'ImageNet' else 5000
        ...
        scale = list(itertools.product([getattr(image_transforms, "image_scale")], list(np.arange(0.9, 1, 0.05))))
        # shear = list(itertools.product([getattr(image_transforms, "image_shear")], list(range(-3, 3))))
        rotation = list(itertools.product([getattr(image_transforms, "image_rotation")], list(range(-10, 10))))

        contrast = list(itertools.product([getattr(image_transforms, "image_contrast")], [0.8 + 0.1 * k for k in range(5)]))
        brightness = list(itertools.product([getattr(image_transforms, "image_brightness")], [10 + 2 * k for k in range(5)]))
        blur = list(itertools.product([getattr(image_transforms, "image_blur")], [k + 1 for k in range(5)]))
...
class Fuzzer(object):
    def __init__(self, params, coverage, blind):
        self.params = params
        self.blind = blind
        self.epoch = 0
    ...
    def is_adversarial(self, image, label, k=1):
        with torch.no_grad():
            ...
            scores = self.coverage.predict(image)
            ...
            for s_i in range(len(S)):
                ...
                for i in range(1, Ps(s_i) + 1):
                    ...
                    if self.isFailedTest(I_new):
                        ...
                        has_gain = self.CoverageGain(delta)
                        nwrong = self.is_adversarial(torch_image, torch_label)[0]

                        if self.blind:
                            has_gain, nwrong = False, 0

                        if has_gain or nwrong > 0:
                            break
...
if __name__ == '__main__':
    ...
    from support.fuzzdatasets import CIFAR10Dataset, ImageNetDataset
    from support.fuzzdatasets import TorchCIFAR10Dataset, TorchImageNetDatasetCov
    import cco
    ...
    parser.add_argument('--model', type=str, default='Qresnet50',
                            choices=['Qresnet50', 'Qvgg16_bn', 'Qdensenet121', 'Qgooglenet', 'Qinception_v3'])
    parser.add_argument('--blind', action='store_true', help='disable coverage feedback')
    ...
    # use test/val data as fuzzing seed
    if args.dataset == 'CIFAR10':
        data_set = CIFAR10Dataset(args, split='test')
    elif args.dataset == 'ImageNet':
        data_set = ImageNetDataset(args, split='val')
    ...
    coverage = cco.PerNeuronCoverageObject(
        args.model, args.metric, args.dataset, args.batch_size, args.image_size
    )

    if coverage.bounds_based:
        if args.dataset == 'CIFAR10':
        ...
        for i, (image, label) in enumerate(tqdm(train_loader)):
            # image = image.cuda()
            coverage.update_extra_params([image], batch_size=train_batch_size)

    print('Initializing...')
    coverage.compile()
    # coverage.calc_and_update(image_batch_list)
    ...
    engine = Fuzzer(args, coverage, args.blind)
