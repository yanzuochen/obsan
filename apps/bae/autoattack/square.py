    ...
    def attack_single_run(self, x, y):
        with torch.no_grad():
            ...
            if self.norm == 'Linf':
                ...
                # print(f'orig margin_min: {self.margin_and_loss(x, y)[0]}')

                while n_defenses < self.n_queries:
                ...
                if n_queries[0] >= self.n_queries:
                    print('{}'.format(self.n_queries),
                        'INIT FAILED',
                        '- success rate={}/{} ({:.2%})'.format(
                        0, 1, 0),
                        '- avg # queries={:.1f}'.format(
                        n_queries[0].mean().item()),
                        '- med # queries={:.1f}'.format(
                        n_queries[0].median().item()),
                        '- defenses={}'.format(n_defenses))
                    return n_queries, x_best

                # print(f'{margin_min=}')

                if (margin_min < 0.0).all():
                    print('{}'.format(int(n_queries[0].item()) + 1),
                        '- success rate={}/{} ({:.2%})'.format(
                        1, 1, 1),
                        '- avg # queries={:.1f}'.format(
                        n_queries[0].mean().item()),
                ...
                for i_iter in tqdm(range(int(n_queries[0].item()), self.n_queries)):
                ...
                for i_iter in tqdm(range(int(n_queries[0].item()), self.n_queries)):
                    ...
                    # print(x_best_curr.shape, x_curr.shape)
                ...
                for i_iter in range(self.n_queries):
                    ...
                    ind_succ = (margin_min <= 0.).nonzero().squeeze()
                    if self.verbose and (ind_succ.numel() != 0 or i_iter == self.n_queries - 1):
                        print('{}'.format(i_iter + 1),
                            '- success rate={}/{} ({:.2%})'.format(
                            ind_succ.numel(), n_ex_total, float(
                            ind_succ.numel()) / n_ex_total),
